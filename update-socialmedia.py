...
A second sheet named "ContactEmail" holds the contact / notification email
in a single "Email" column (header + one value row). It is emitted as
window.CONTACT_EMAIL; the frontend uses it for the Web3Forms recipient and
the address shown in the success message and terms. Blank/missing -> null,
and the frontend falls back to its hardcoded default.
...
def parse_contact_email(rows):
    """Return the first non-empty value under the 'Email' column, or None."""
    email_col = None
    for cells in rows:
        if email_col is None:
            for col, val in cells.items():
                if str(val).strip().lower() == "email":
                    email_col = col
                    break
            continue
        val = str(cells.get(email_col, "") or "").strip()
        if val:
            return val
    return None
...
def read_xlsx(path):
    ...
    with zipfile.ZipFile(path) as z:
        ...
        socials = parse_socials(read_rows(z, paths[sheet], strings)) if sheet else {}

        # The sheet named "ContactEmail" (case-insensitive), if present.
        email_sheet = next((n for n in names if n.strip().lower() == "contactemail"), None)
        contact_email = parse_contact_email(read_rows(z, paths[email_sheet], strings)) if email_sheet else None

    return socials, contact_email


def main():
    socials, contact_email = read_xlsx(XLSX)
    with open(OUT, "w", encoding="utf-8", newline="\n") as f:
        ...
        f.write("window.CONTACT_EMAIL = " + json.dumps(contact_email) + ";\n")
    email_note = contact_email if contact_email else "(none — frontend default)"
    print(f"Wrote {len(socials)} currency social-link set(s); contact email: {email_note}")
